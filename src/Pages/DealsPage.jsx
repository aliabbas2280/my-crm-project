...
const DealsPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    const payload = { ...formData, clientName: client?.name || '', userId: user?.id, userName: user?.name };
    try {
      if (editingDeal) await dealsAPI.update(editingDeal.id, payload);
      else await dealsAPI.create(payload);
      await activitiesAPI.create({ type: editingDeal ? 'deal_updated' : 'deal_created', message: editingDeal ? `Deal "${payload.title}" updated` : `New deal "${payload.title}" created`, userId: user?.id });
      fetchDeals();
      closeModal();
    } catch { }
  };
  ...
  const handleEdit = (deal) => {
    setEditingDeal(deal);
    setFormData({ title: deal.title, clientId: deal.clientId, value: deal.value, status: deal.status, expectedCloseDate: deal.expectedCloseDate || '' });
    setShowModal(true);
  };
  ...
  const closeModal = () => {
    setShowModal(false);
    setEditingDeal(null);
    setFormData({ title: '', clientId: '', value: '', status: 'Lead', expectedCloseDate: '' });
  };
  ...
  const renderRow = (deal) => (
    <tr key={deal.id}>
      ...
      <td>
        <Form.Select size="sm" value={deal.status} onChange={(e) => handleStatusChange(deal, e.target.value)}>
  ...
  return (
    <>
      <button type="button" className="hamburger-menu d-lg-none" onClick={() => setSidebarOpen(!sidebarOpen)}>
